...
// Reels helpers
export const getAllReels = (): VideoProject[] => {
  return allReels.sort(
    (a, b) => new Date(b.publish_date).getTime() - new Date(a.publish_date).getTime()
  );
};

export const getReelsByCategory = (category: string): VideoProject[] => {
  if (category === "All") return getAllReels();

  const filtered = allReels.filter((r) => r.category.includes(category));
