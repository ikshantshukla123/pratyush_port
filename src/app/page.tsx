      ...
      <section className="py-20 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <p className="text-gray-300 text-lg max-w-3xl mx-auto mb-8">
              Short-form highlights and promos I crafted for Instagram.
            </p>
          </motion.div>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {getAllReels().map((reel, index) => (
              <motion.div
                key={reel.id}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                <a href={reel.video_link} target="_blank" rel="noreferrer">
                  <GlassmorphismCard className="p-6 h-full group">
                    <div className="space-y-4 h-full flex flex-col">
                      <div className="relative overflow-hidden rounded-lg">
                        {/* Use local placeholder images; if you add actual thumbnails to public/, replace src accordingly */}
                        <Image
                          src={
                            reel.cover_image.startsWith("/")
                              ? reel.cover_image
                              : `/project-images/${reel.cover_image}.png`
                          }
                      ...
                      <div className="flex-1">
                        <h3 className="text-xl font-semibold mb-2 text-white group-hover:text-pink-400 transition-colors line-clamp-2">
                          {reel.video_title}
                        </h3>
                        <p className="text-gray-300 text-sm mb-4 line-clamp-3">
                        ...
                        <div className="flex items-center space-x-4 text-xs text-gray-400 mb-4">
                          <div className="flex items-center space-x-1">
                            <User size={12} />
                            <span>{reel.client_name}</span>
                          </div>
                          <div className="flex items-center space-x-1">
                            <Clock size={12} />
                            <span>{new Date(reel.publish_date).toLocaleDateString()}</span>
                          </div>
                        </div>

                        <div className="flex flex-wrap gap-2 mb-4">
                          {reel.category.slice(0, 3).map((cat) => (
                            <Badge
                              key={cat}
                              variant="outline"
                              className="text-xs border-gray-600 text-gray-300"
                      ...
                      <div className="pt-4 border-t border-white/20">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-2">
                            <Image
                              src={reel.client_image || "/placeholder.svg"}
                              alt={reel.client_name}
                              width={24}
                              height={24}
                              className="rounded-full"
                            />
                            <span className="text-sm text-gray-400">{reel.client_name}</span>
                          </div>
                          <Button
                            size="sm"
                            className="bg-white/10 text-white border-white/20 hover:bg-white/20 cursor-pointer"
